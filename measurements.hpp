  ...
  for (std::size_t flavor1 = 0; flavor1 < n_flavors; ++flavor1) {
    for (std::size_t site1 = 0; site1 < n_site; ++site1) {
      for (std::size_t site2 = 0; site2 < n_site; ++site2) {
        ...
        if (flavor1==0 && site1==site2 && results[Wk_imag_name.str().c_str()].has_tau()) {
          std::vector<double> real_tau = results[Wk_real_name.str().c_str()].template tau<std::vector<double> >();
          std::vector<double> imag_tau = results[Wk_imag_name.str().c_str()].template tau<std::vector<double> >();
          std::cout << " Autocorrelation time of onsite part of Wk[iomega_n] : flavor= " <<flavor1<<" site= " << site1 << std::endl;
          for (unsigned int w = 0; w < n_matsubara_meas; ++w) {
            std::cout << w << " " << real_tau[w] << " " << imag_tau[w] << std::endl;
          }
        }
