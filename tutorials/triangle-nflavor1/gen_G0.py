...
f = open('G0_TAU.txt','w')
print(1, 2*norb, ndiv_tau+1, vbeta, file=f)

for iorb, ispin, jorb, jspin, itau in product(list(range(norb)), list(range(2)), list(range(norb)), list(range(2)), list(range(ndiv_tau+1))):
    if ispin == jspin:
         print(0, 2*iorb+ispin, 2*jorb+jspin, itau, cut_small_value(G0_tau[itau,iorb,0,jorb,0].real), cut_small_value(G0_tau[itau,iorb,0,jorb,0].imag), file=f)
    else:
         print(0, 2*iorb+ispin, 2*jorb+jspin, itau, 0.0, 0.0, file=f)
f.close()
