    ...
    namespace ctint {
            ...
            for (std::size_t site1 = 0; site1 < n_site; ++site1) {
              for (std::size_t site2 = 0; site2 < n_site; ++site2) {
                ...
                for (unsigned int w = 0; w < n_matsubara_meas; ++w) {
                  green_matsubara_measured(w, site1, site2, flavor1) =
                    -std::complex<double>(mean_real[w], mean_imag[w]) / (sign * beta);
                }
              }
            }
          }
        ...
        */
        ...
        void evaluate_selfenergy_measurement_legendre(const typename alps::accumulators::result_set &results,
                                                      ...
                                                      std::vector<double> &densities,
                                                      const double &beta, std::size_t n_site,
                                                      std::size_t n_flavors, std::size_t n_matsubara, std::size_t n_tau,
                                                      std::size_t n_legendre) {
          green_matsubara_measured.clear();
        ...
        void compute_greens_functions(const typename alps::accumulators::result_set &results,
          ...
          typename SOLVER_TYPE::matsubara_green_function_t sigma_green_matsubara_measured(n_matsubara, n_site,
                                                                                          n_flavors);
          ...
          std::cout << "average sign was: " << results["Sign"].template mean<double>() << " error: "
                    << results["Sign"].template error<double>() << std::endl;
        ...
        if (n_matsubara_measurements > 0) {
          ...
          */
          ...
          if (n_legendre > 0) {
            ...
            typename SOLVER_TYPE::matsubara_green_function_t sigma_green_matsubara_measured_l(n_matsubara, n_site,
                                                                                              n_flavors);

            evaluate_selfenergy_measurement_legendre<SOLVER_TYPE>(results, green_matsubara_measured_l,
                                                                  sigma_green_matsubara_measured_l,
                                                                  green_itime_measured_l,
                                                                  bare_green_matsubara, densities,
                                                                  beta, n_site, n_flavors, n_matsubara, n_tau,
                                                                  n_legendre);
